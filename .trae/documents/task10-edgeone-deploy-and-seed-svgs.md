...
| 8 | 逻辑 | 保留两套边缘函数（SiliconFlow + Edge AI），两模型输出可能不一致 | 注明 demo 主用 Edge AI；helpers 复用同一 SYSTEM_PROMPT 保证解析格式一致 |
...
v1 成立且保留的部分：三 Workstream 结构、HashRouter 选择（优于 rewrites）、tsconfig include 分析、防御性 content 提取、复用 SYSTEM_PROMPT、保留 api/llm.ts 作 Vercel 备用。
